...
{#if !screenSmaller}
  ...
  >
    <div class="mb-6">
      {#if screenLarge}
        <img
          src="/images/vocal-icon-square.png"
          alt="Vocal Icon"
          class="rounded-full drop-shadow-md drop-shadow-indigo-500/50"
        />
      {:else}
        <h2 class="text-3xl font-bold text-white tracking-wider">Vocal</h2>
      {/if}
      {#if !screenLarge}<p class="text-xl text-purple-300 mt-2">
    ...
    <nav class="space-y-1">
      {#each navItems as item}
        <a
          href={item.href}
          class="flex items-center p-3 rounded-md transition-colors text-gray-300 {isActive(
            item.href,
          )
            ? ''
        ...
        >
          <svelte:component
            this={item.icon}
            class="h-5 w-5 {screenLarge ? 'ml-0.5' : 'mr-3'}"
          />
          {#if !screenLarge}
    ...
    <div class="mt-auto pt-4 border-t border-[#252025]">
      {#if screenLarge}
        <button
          class="w-full flex items-center gap-3 p-3 rounded-md text-gray-300 hover:bg-[#32353b] hover:text-purple-300 hover:text-white transition-colors"
        >
      ...
      {:else}
        <a href="/users/{user?.username}">
          <div class="flex items-center gap-3 p-3">
            <img
              src={avatarSrc}
              alt="AV"
              class="h-10 w-10 rounded-sm ring-2 ring-purple-500"
            />
            <div>
              <p class="font-medium text-white text-xl" use:twemojify>
                {user?.displayName || ""}
              </p>
        ...
        <button
          class="w-full flex items-center gap-3 p-3 rounded-md text-gray-300 hover:bg-purple-900/40 hover:text-white transition-colors"
        >
...
<div
  class="{screenSmaller ? 'hidden' : ''} {screenLarge ? 'w-20' : 'w-72'}"
></div>
