...
<div class="flex flex-col min-h-screen">
    <header
        class="sticky top-0 z-10 bg-vocal_strong border-b border-[#2d2249] p-8"
        id="profileHeader"
    >
            ...
            >
                    ...
                    >
                        <Dialog.Header>
                            <Dialog.Title class="text-2xl"
                                >{user.displayName || user.username}'s following</Dialog.Title
                            >
                            <Dialog.Description>
                                {#each follow.ingData as f}
                                    <Dialog.Close>
                                        <a
                                            href="/users/{f.discordId}"
                                            class="flex items-center gap-2 mt-3 text-xl"
                                        >
                                            <img
                                                src={f.avatarUrl || '/images/fallback-pfp.jpg'}
                                                alt={f.username}
                ...
                <Dialog.Root>
                    ...
                    >
                        <Dialog.Header>
                            <Dialog.Title class="text-2xl"
                                >{user.displayName || user.username}'s followers</Dialog.Title
                            >
                            <Dialog.Description>
                    ...
                    {#if posts && posts.userReplies.length > 0}
                        {#each posts.userReplies as reply}
                            <Post 
                                post={reply}
                                postAuthor={user}
                                postLikes={reply.replyLikes.filter(p => p.parent_post === reply._id)}
                                postReplies={reply.nestedReplies.filter(p => p.parent_post === reply._id)}
                                user={user}
                                reply={true}
                            />
                        {/each}
