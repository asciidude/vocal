...
export const POST: RequestHandler = async ({ request, fetch }) => {
  try {
    ...
    if (subscribed) {
      return json({ success: false, message: "You are already subscribed" }, { status: 400 });
    }
        ...
        <tr>
          <td align="center" style="padding:32px 16px;">
            <table width="100%" style="max-width:600px; background:#121212; border-radius:12px; overflow:hidden;">
              <tr>
                <td>
                  <img src="https://vocal.wtf/images/ms-banner-success.png" alt="You're subscribed" style="width:100%; display:block;" />
                </td>
              </tr>
              <tr>
                <td style="padding:24px 28px;">
                  <h1 style="margin:0 0 12px; font-size:22px; font-weight:600;">You’re subscribed 🎉</h1>
    ...
    const res = await fetch(endpoint, {
      ...
      body: JSON.stringify({ content: `${email} has subscribed to the mailing list.` })
    });
    ...
    return json({ success: true, message: "You have been subscribed successfully!" }, { status: 200 });
