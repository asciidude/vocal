...
export const POST: RequestHandler = async ({ params, request, locals }) => {
    ...
    try {
        const existingLike = await LikeModel.exists({ parent_post: postId, author: user._id });
        ...
        if (!existingLike) {
            ...
            await LikeModel.create({ parent_post: postId, author: user._id });
            newlyLiked = true;
        } else {
            if (postType === 'post') {
                const updatedVector = { ...(userDoc.userInterestVectors || {}) };
                for (const [token, weight] of Object.entries(post.postVectors || {})) {
                    if (updatedVector[token] !== undefined) {
                        ...
                        if (updatedVector[token] < 0.001) delete updatedVector[token];
                    }
                }
                const mag = Math.sqrt(Object.values(updatedVector).reduce((s, v) => s + v * v, 0)) || 1;
                for (const k in updatedVector) updatedVector[k] = updatedVector[k] / mag;
            ...
            await LikeModel.deleteOne({ parent_post: postId, author: user._id });
            newlyLiked = false;
        }

        const likeCount = await LikeModel.countDocuments({ parent_post: postId });
